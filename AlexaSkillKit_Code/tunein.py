...
class TuneIn:
    ...
    def __init__(self, partnerid=None, serial=None, locale="en-GB", formats=None, https=True, debug=False):
        if https is False:
            self._protocol = 'http://'
        else:
    ...
    def call_tunein(self, method, params=None):
    ...
    def result_fault(self, result):
        if ('fault' in result['head']):
            return result['head']['fault']
        else:
            return ''

    def result_fault_code(self, result):
    ...
    def account_auth(self, username, password):
        ...
        result = self.call_tunein('Account.ashx', params)
    ...
    def config_time(self):
        ...
        result = self.call_tunein('Config.ashx', params)
        if (self.result_ok(result)):
            raise TuneIn.TuneInError(self.result_status(
                result), self.result_fault(result), self.result_fault_code(result))
        else:
    ...
    def describe_nowplaying(self, id):
        ...
        params = [{'param': 'c', 'value': 'nowplaying'}, {'param':
                                                          'id', 'value': id}]
        result = self.call_tunein('Describe.ashx', params)
        if (self.result_ok(result)):
            raise TuneIn.TuneInError(self.result_status(
                result), self.result_fault(result), self.result_fault_code(result))
        else:
    ...
    def options(self, id):
        ...
        '''
        params = [{'param': 'id', 'value': 'id'}]
        result = self.call_tunein('Options.ashx', params)
        if (self.result_ok(result)):
            raise TuneIn.TuneInError(self.result_status(
                result), self.result_fault(result), self.result_fault_code(result))
        else:
    ...
    def preset_add(self, username=None, password=None, folderid=None, id=None, url=None, presetnumber=None, name=None):
        ...
        result = self.call_tunein('Preset.ashx', params)
        if (self.result_ok(result)):
            raise TuneIn.TuneInError(self.result_status(
                result), self.result_fault(result), self.result_fault_code(result))
        else:
    ...
    def tune_show(self, id, flatten=None, category='pbrowse', filter=''):
        ...
        result = self.call_tunein('Tune.ashx', params)
        if (self.result_ok(result)):
            raise TuneIn.TuneInError(self.result_status(
                result), self.result_fault(result), self.result_fault_code(result))
        return result['body']